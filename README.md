...
Com a regulação carregada aparecem mais dois filtros. **Dia do atendimento** lista cada data da agenda com a quantidade de agendamentos, e traz cada mês inteiro no topo da lista — escolher um dia recorta o painel todo: as APACs, o trilho do ciclo, a situação dos agendamentos, as unidades solicitantes e a lista de atendidos sem APAC. Quando o paciente tem mais de um agendamento, a linha passa a mostrar o do dia escolhido. **Regulação** traz os recortes prontos: chave fechada, chave aberta, atendido com APAC não autorizada, com agendamento, sem agendamento, paciente faltou, agendamento pendente ou futuro.
...
| Regra da chave fechada | função `aplicarAgendamentos` e `leituraChave` |
| Como o dia do atendimento é recortado | funções `combinaDia` e `montarSelectDia` |
